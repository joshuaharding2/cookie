...
Game.Launch=function()
{
	...
	'<div class="listing">'+loc("<b>Disclaimer</b>")+'</div>'+
	'<div class="listing">'+loc("")+'</div>'+
	'<div class="listing">'+loc("This version of Cookie Clicker has been modified by Joshua Harding (2025). These changes are unofficial and may introduce bugs, glitches, or other issues.")+'</div>'+
	'<div class="listing">'+loc("")+'</div>'+
	'<div class="listing">'+loc("The original Cookie Clicker was created by Orteil and Opti. I am not the original creator, I do not claim ownership of Cookie Clicker, and I am not profiting from this project.")+'</div>'+
	'<div class="listing">'+loc("")+'</div>'+
	'<div class="listing">'+loc("This modified version was made purely for fun and personal use. I only intend to share it privately with friends. If this version is found on a website or distributed widely, that was not my intention.")+'</div>'+
	'<div class="listing">'+loc(" <br> <br> ")+'</div>'+
	...
	Game.Init=function()
	{
		...
		l('versionNumber').innerHTML='v. '+Game.version+" modded"+(!App?('<div id="httpsSwitch" style="cursor:pointer;display:inline-block;background:url(img/'+(Game.https?'lockOn':'lockOff')+'.png);width:16px;height:16px;position:relative;top:4px;left:0px;margin:0px -2px;"></div>'):'')+(Game.beta?' <span style="color:#ff0;">beta</span>':'');
		...
		Game.DefaultPrefs=function()
		{
			...
			//modded prefs
			Game.prefs.autoclickerCps = 1;
			Game.prefs.autoclickerHotkey = "`";
			Game.prefs.autoclickerOn = 0; // OFF by default
			Game.prefs.autoclickerCpsCap = 0;
		}
		Game.DefaultPrefs();
		...
		Game.WriteSave=function(type)
		{
			...
			(Game.prefs.autoclickerOn?'1':'0')+
			'';
			str2 += ';'+[
				Game.prefs.autoclickerCps,
				Game.prefs.autoclickerHotkey,
				Game.prefs.autoclickerCpsCap
			].join(',');
			str2=pack3(str2);
			str+=str2+'|';
			if (type==3) str+='\n\nMisc game data';
				...
				else
				{
					...
					if (version>=1)
					{
						...
						if (version<1.0503) spl=str[3].split('');
							...
							// support extra prefs after a semicolon
							var prefsParts = str[3].split(';');
							spl = prefsParts[0].split('');
							// custom prefs (after ;)
							if (prefsParts[1]) {
								var extra = prefsParts[1].split(',');
								Game.prefs.autoclickerCps = parseInt(extra[0]) || 0;
								Game.prefs.autoclickerHotkey = extra[1] || '`';
								Game.prefs.autoclickerCpsCap = parseInt(extra[2]) || 0;
							} else {
								Game.prefs.autoclickerCps = 0;
								Game.prefs.autoclickerHotkey = '`';
								Game.prefs.autoclickerCpsCap = 0;
							}
						}
						...
						Game.prefs.autoclickerOn = spl[27] ? parseInt(spl[27]) : 0;
						if(Game.prefs.autoclickerOn)autoclicker.start();
						BeautifyAll();
		...
		Game.ClickCookie=function(e,amount, forceCenter)
		{
			var now=Date.now();
			...
			else
			{
				...
				var px = Game.mouseX, py = Game.mouseY;
				if (forceCenter) {
				try {
					var cookieEl = document.getElementById('bigCookie');
					var gameEl = document.getElementById('game') || document.getElementById('wrapper');
					if (cookieEl && gameEl) {
					var crect = cookieEl.getBoundingClientRect();
					var grect = gameEl.getBoundingClientRect();
					px = Math.round(crect.left - grect.left + crect.width/2);
					py = Math.round(crect.top  - grect.top  + crect.height/2.2);
					}
				} catch(err) {
					// fallback to Game.mouseX/Y if anything goes wrong
					px = Game.mouseX; py = Game.mouseY;
				}
				}
		...
		Game.UpdateMenu=function()
		{
			...
			if (Game.onMenu=='prefs')
			{
				...
				str+=
					...
					'</div>'+
				'</div>';
				if (Game.Has('Autoclicker'))
				{ str+=
						'<div class="block" style="padding:0px;margin:8px 4px;">'+
						'<div class="subsection" style="padding:0px;">'+
							'<div class="title">Autoclicker</div>'+
							...
							// CPS input
							'<div class="listing">'+
								'<label>CPS: <input type="number" id="autoclickerCPS" value="'+Game.prefs.autoclickerCps+'" min="1" style="width:60px;"></label>'+
								' <a class="option" onclick="Game.prefs.autoclickerCps=parseInt(l(\'autoclickerCPS\').value)||1;if(window.autoclicker) autoclicker.setCPS(Game.prefs.autoclickerCps);Game.UpdateMenu();">Set</a>'+
								' <a class="option" onclick="Game.prefs.autoclickerCps=(window.autoclicker?Game.prefs.autoclickerCpsCap:0);if(window.autoclicker) autoclicker.setCPS(Game.prefs.autoclickerCps);Game.UpdateMenu();">Max</a>'+
								' <small>(Max: '+Game.prefs.autoclickerCpsCap+')</small>'+
							'</div>'+

							// Hotkey input
							'<div class="listing">'+
								'<label>Hotkey: <input type="text" id="autoclickerHotkey" value="'+Game.prefs.autoclickerHotkey+'" maxlength="1" style="width:40px;text-align:center;"></label>'+
								' <a class="option" onclick="Game.prefs.autoclickerHotkey=l(\'autoclickerHotkey\').value.toUpperCase()||\'`\' ;if(window.autoclicker) autoclicker.hotkey=Game.prefs.autoclickerHotkey;Game.Notify(\'Autoclicker\',\'Hotkey set to \'+Game.prefs.autoclickerHotkey,[16,5]);Game.UpdateMenu();">Set</a>'+
							'</div>'+
		...
		//Joshua Harding was here
		var order = 2000;
		new Game.Upgrade("Autoclicker",loc("Unlocks the autoclicker (Added by Joshua Harding).")+'<q>Us</q>',1,[0, 0, 'img/autoclickerIcons.png']).buyFunction = function() {if (window.autoclicker) {window.autoclicker.purchased = true;Game.prefs.autoclickerCpsCap = Math.max(Game.prefs.autoclickerCpsCap, 0);Game.prefs.autoclickerOn = 0;Game.prefs.autoclickerCps = Math.max(Game.prefs.autoclickerCpsCap, 0);Game.prefs.autoclickerHotkey = "`";Game.Notify("Autoclicker unlocked!", "You can now enable it in Options.", [0, 0, 'img/autoclickerIcons.png']);Game.UpdateMenu();}};
		new Game.Upgrade("Rusty Clickspring",loc("The autoclicker can click up to <b>1 CPS</b>")+'<q>An ancient, squeaky spring that clicks… eventually.</q>',10,[1, 0, 'img/autoclickerIcons.png']).buyFunction = function() {if (window.autoclicker) {Game.prefs.autoclickerCpsCap = Math.max(Game.prefs.autoclickerCpsCap, 1);Game.UpdateMenu();}};
		new Game.Upgrade("Mechanical Finger",loc("The autoclicker can click up to <b>3 CPS</b>")+'<q>A crude robotic digit that presses the mouse for you.</q>',100,[2, 0, 'img/autoclickerIcons.png']).buyFunction = function() {if (window.autoclicker) {Game.prefs.autoclickerCpsCap = Math.max(Game.prefs.autoclickerCpsCap, 3);Game.UpdateMenu();}};
		new Game.Upgrade("Hydraulic Tapper",loc("The autoclicker can click up to <b>5 CPS</b>")+'<q>Uses water pressure to deliver consistent cookie clicks.</q>',1000,[3, 0, 'img/autoclickerIcons.png']).buyFunction = function() {if (window.autoclicker) {Game.prefs.autoclickerCpsCap = Math.max(Game.prefs.autoclickerCpsCap, 5);Game.UpdateMenu();}};
		new Game.Upgrade("Servo Clickbot",loc("The autoclicker can click up to <b>7 CPS</b>")+'<q>A nimble, reliable little helper with a taste for efficiency.</q>',10000,[0, 1, 'img/autoclickerIcons.png']).buyFunction = function() {if (window.autoclicker) {Game.prefs.autoclickerCpsCap = Math.max(Game.prefs.autoclickerCpsCap, 7);Game.UpdateMenu();}};
		new Game.Upgrade("Optical Clicker",loc("The autoclicker can click up to <b>10 CPS</b>")+'<q>Employs laser guidance for pixel-perfect precision.</q>',100000,[1, 1, 'img/autoclickerIcons.png']).buyFunction = function() {if (window.autoclicker) {Game.prefs.autoclickerCpsCap = Math.max(Game.prefs.autoclickerCpsCap, 10);Game.UpdateMenu();}};
		new Game.Upgrade("Quantum Pointer",loc("The autoclicker can click up to <b>15 CPS</b>")+'<q>Clicks across multiple timelines — most of them successfully.</q>',1000000,[2, 1, 'img/autoclickerIcons.png']).buyFunction = function() {if (window.autoclicker) {Game.prefs.autoclickerCpsCap = Math.max(Game.prefs.autoclickerCpsCap, 15);Game.UpdateMenu();}};
		new Game.Upgrade("Cookie Compactor Arm",loc("The autoclicker can click up to <b>20 CPS</b>")+'<q>A heavy-duty robot arm designed for repetitive tasks… and snacks.</q>',10000000,[3, 1, 'img/autoclickerIcons.png']).buyFunction = function() {if (window.autoclicker) {Game.prefs.autoclickerCpsCap = Math.max(Game.prefs.autoclickerCpsCap, 20);Game.UpdateMenu();}};
		new Game.Upgrade("A.I. Cursor Network",loc("The autoclicker can click up to <b>30 CPS</b>")+'<q>Thousands of neural nets collaborating to optimize clicking yield.</q>',1000000000,[0, 2, 'img/autoclickerIcons.png']).buyFunction = function() {if (window.autoclicker) {Game.prefs.autoclickerCpsCap = Math.max(Game.prefs.autoclickerCpsCap, 30);Game.UpdateMenu();}};
		new Game.Upgrade("Entropic Fingertip",loc("The autoclicker can click up to <b>40 CPS</b>")+'<q>Collapses local entropy fields into pure click energy.</q>',1000000000000,[1, 2, 'img/autoclickerIcons.png']).buyFunction = function() {if (window.autoclicker) {Game.prefs.autoclickerCpsCap = Math.max(Game.prefs.autoclickerCpsCap, 40);Game.UpdateMenu();}};
		new Game.Upgrade("Singularity Click Core",loc("The autoclicker can click up to <b>50 CPS</b>")+'<q>It clicks before you even think about clicking.</q>',1000000000000000,[2, 2, 'img/autoclickerIcons.png']).buyFunction = function() {if (window.autoclicker) {Game.prefs.autoclickerCpsCap = Math.max(Game.prefs.autoclickerCpsCap, 50);Game.UpdateMenu();}};
		new Game.Upgrade("Chrono-Looped Cursor",loc("The autoclicker can click up to <b>75 CPS</b>")+'<q>A mouse trapped in a perfect loop of eternal clicking.</q>',1000000000000000000,[3, 2, 'img/autoclickerIcons.png']).buyFunction = function() {if (window.autoclicker) {Game.prefs.autoclickerCpsCap = Math.max(Game.prefs.autoclickerCpsCap, 75);Game.UpdateMenu();}};
		new Game.Upgrade("Transdimensional Tapper",loc("The autoclicker can click up to <b>100 CPS</b>")+'<q>Harnesses the power of alternate universes to increase CPS.</q>',1000000000000000000000,[0, 3, 'img/autoclickerIcons.png']).buyFunction = function() {if (window.autoclicker) {Game.prefs.autoclickerCpsCap = Math.max(Game.prefs.autoclickerCpsCap, 100);Game.UpdateMenu();}};
		new Game.Upgrade("The Great Clickformer",loc("The autoclicker can click up to <b>150 CPS</b>")+'<q>A mythical being said to have created the first cookie with a single click.</q>',1000000000000000000000000,[1, 3, 'img/autoclickerIcons.png']).buyFunction = function() {if (window.autoclicker) {Game.prefs.autoclickerCpsCap = Math.max(Game.prefs.autoclickerCpsCap, 150);Game.UpdateMenu();}};
		new Game.Upgrade("Cookieverse Interface",loc("The autoclicker can click up to <b>200 CPS</b>")+'<q>Connects directly to the cookie fabric of reality.</q>',1000000000000000000000000000,[2, 3, 'img/autoclickerIcons.png']).buyFunction = function() {if (window.autoclicker) {Game.prefs.autoclickerCpsCap = Math.max(Game.prefs.autoclickerCpsCap, 200);Game.UpdateMenu();}};
		new Game.Upgrade("Omni-Click Engine",loc("The autoclicker can click up to <b>250 CPS</b>")+'<q>The final click. The first click. All clicks.</q>',1000000000000000000000000000000,[3, 3, 'img/autoclickerIcons.png']).buyFunction = function() {if (window.autoclicker) {Game.prefs.autoclickerCpsCap = Math.max(Game.prefs.autoclickerCpsCap, 250);Game.UpdateMenu();}};
	...
	Game.Logic=function()
	{
		...
		if (!Game.OnAscend && Game.AscendTimer==0)
		{
			...
			if (Game.T%(Game.fps*5)==0 && Game.ObjectsById.length>0)//check some achievements and upgrades
			{
				...
				if (Game.handmadeCookies>=1) Game.Unlock("Autoclicker");
				if (Game.handmadeCookies>=1) Game.Unlock("Rusty Clickspring");
				if (Game.handmadeCookies>=10) Game.Unlock("Mechanical Finger");
				if (Game.handmadeCookies>=100) Game.Unlock("Hydraulic Tapper");
				if (Game.handmadeCookies>=1000) Game.Unlock("Servo Clickbot");
				if (Game.handmadeCookies>=10000) Game.Unlock("Optical Clicker");
				if (Game.handmadeCookies>=100000) Game.Unlock("Quantum Pointer");
				if (Game.handmadeCookies>=1000000) Game.Unlock("Cookie Compactor Arm");
				if (Game.handmadeCookies>=100000000) Game.Unlock("A.I. Cursor Network");
				if (Game.handmadeCookies>=100000000000) Game.Unlock("Entropic Fingertip");
				if (Game.handmadeCookies>=100000000000000) Game.Unlock("Singularity Click Core");
				if (Game.handmadeCookies>=100000000000000000) Game.Unlock("Chrono-Looped Cursor");
				if (Game.handmadeCookies>=100000000000000000000) Game.Unlock("Transdimensional Tapper");
				if (Game.handmadeCookies>=100000000000000000000000) Game.Unlock("The Great Clickformer");
				if (Game.handmadeCookies>=100000000000000000000000000) Game.Unlock("Cookieverse Interface");
				if (Game.handmadeCookies>=100000000000000000000000000000) Game.Unlock("Omni-Click Engine");
...
// wrap Game.Launch so our code runs after the game finishes launching
(function(){
    var oldLaunch = Game.Launch;
    Game.Launch = function(){
        oldLaunch.apply(this, arguments);
        ...
        function toggleConsole() {
            ...
            document.getElementById('versionNumber').style.display = isVisible ? "block" : "none";
